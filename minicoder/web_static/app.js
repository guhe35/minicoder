...
function translateReason(reason = "") {
  if (reason.startsWith("No files were modified")) {
    return "本次任务没有修改文件，因此不需要执行修改后验证。";
  }
  if (reason.startsWith("Files were modified, but no recognized")) {
    return "文件已经修改，但最新修改之后尚未运行可识别的验证命令。";
  }
...
function renderReport(result) {
  ...
  setVerdict(report.status, STATUS_LABELS[report.status] || report.status);
  ...
  if (!report.verification_runs.length) {
    ...
    report.verification_runs.forEach((run) => {
      const row = document.createElement("div");
      row.className = "evidence-run";
      const type = document.createElement("span");
      type.className = "evidence-type";
      type.textContent = EVIDENCE_LABELS[run.kind] || run.kind;
...
function handleEvent(message) {
  ...
  if (event === "run_started") {
    ...
    addTrace(
      ok ? "success" : "error",
      ok ? "工具结果" : "工具错误",
      `${TOOL_LABELS[data.tool] || data.tool}${ok ? "执行成功" : "执行失败"}`,
      compact(data.result),
      data.result,
    );
    ...
    addTrace(
      data.status === "verified" ? "success" : "model",
      "控制器",
      `任务结束：${STATUS_LABELS[data.status] || data.status}`,
      `智能体在第 ${data.step} 轮后停止。`,
    );
...
clearButton.addEventListener("click", () => {
  if (!runButton.disabled) resetTrace();
});
